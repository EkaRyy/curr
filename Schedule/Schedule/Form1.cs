...
namespace Schedule
{
    public partial class FormMain : Form
    {
        ...
        private void Pairs2Grid()
        {
            ...
            dt.Columns.Add(new DataColumn("O", typeof(Time)));

            foreach (var pair in Time.getAllPairs())
            {
        ...
        private void Elems2Grid()
        {
            ...
            foreach (var item in DM.elems.FindAll())
            {
                ...
                if (comboFiltGroups.SelectedItem != null)
                    if (((Course)comboFiltGroups.SelectedItem).Id != item.group.Id) z = false;
                ...
                if (comboFiltTeachers.SelectedItem != null)
                    if (((Professor)comboFiltTeachers.SelectedItem).Id != item.teacher.Id) z = false;
        ...
        {
            Professor obj = getObj<Professor>(dgvTeachers,null);
            if (obj == null)
            {
        ...
        {
            Course obj = getObj<Course>(dgvGroups, null);
            if (obj == null)
            {
        ...
        private void comboTypesOne_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            if (comboTypesOne.SelectedIndex == IDX_PAIR)            
                comboFiltOne.Items.AddRange(Time.getAllPairs().ToArray());
        ...
        private void goPairOne(Time pair)
        {
            painterone.setFilt(DM.elems.FindAll(),(el) => el.pair.isPairEqual(pair),typeof(Time));
            panelOneAxis.Refresh();
        }

        private void goTeacherOne(Professor teacher)
        {
            painterone.setFilt(DM.elems.FindAll(), (el) => el.teacher.Id == teacher.Id, typeof(Professor));
            panelOneAxis.Refresh();
        }

        private void goGroupOne(Course group)
        {
            painterone.setFilt(DM.elems.FindAll(), (el) => el.group.Id == group.Id, typeof(Course));
            panelOneAxis.Refresh();
        }
        ...
        {
            if (comboTypesOne.SelectedIndex == IDX_PAIR)
                goPairOne((Time)comboFiltOne.SelectedItem);
            if (comboTypesOne.SelectedIndex == IDX_TEACHER)
                goTeacherOne((Professor)comboFiltOne.SelectedItem);
            if (comboTypesOne.SelectedIndex == IDX_ROOM)
                goRoomOne((Room)comboFiltOne.SelectedItem);
            if (comboTypesOne.SelectedIndex == IDX_GROUP)
                goGroupOne((Course)comboFiltOne.SelectedItem);
        }
        ...
        {
            Time obj = getObj<Time>(dgvPairs, null);
        ...
        private void butSelAxis1_Click(object sender, EventArgs e)
        {
            ...
            if (comboAxisType1.SelectedIndex == IDX_PAIR)
                FormSelAxis.fillFilt(Time.getAllPairs().ToList<Axis>(), filt1);            
        ...
        private void butSelAxis2_Click(object sender, EventArgs e)
        {
            ...
            if (comboAxisType2.SelectedIndex == IDX_PAIR)
                FormSelAxis.fillFilt(Time.getAllPairs().ToList<Axis>(), filt2);
